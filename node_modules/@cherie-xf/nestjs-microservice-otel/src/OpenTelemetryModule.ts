...
import { TraceClientProxy } from './Trace/TraceClientProxy';
...
import { RpcTraceContextPropagator } from './Trace/RpcTraceContextPropagator';
...
export class OpenTelemetryModule implements NestModule {
  ...
  static async forRoot(
    ...
    return {
      ...
      providers: [
        ...
        this.buildTracerProvider(configuration),
        this.buildMeter(),
        this.buildTracer(),
        {
  ...
  private static buildTracerProvider(
    configuration?: Partial<OpenTelemetryModuleConfig>,
  ): FactoryProvider {
    return {
      ...
      useFactory: async () => {
        ...
        const provider = new NodeTracerProvider(configuration);
        provider.addSpanProcessor(configuration.spanProcessor);
        // provider.register();
        provider.register({ propagator: new RpcTraceContextPropagator() });
  ...
  static async forRootAsync(
    ...
    return {
      ...
      exports: [TraceService, TraceClientProxy, MetricService, Meter, Tracer],
    };
  }
  private static buildAsyncTraceProvider(): FactoryProvider {
    return {
      provide: NodeTracerProvider,
      useFactory: async (config) => {
        ...
        provider.addSpanProcessor(config.spanProcessor);
        // provider.register();
        provider.register({ propagator: new RpcTraceContextPropagator() });
