...
dowork() {
    ...
    {
        run_code_cmd="(cat /app/input/$1.in | /user-solve/main) 1> ${output_path} 2>${error_path}"
        ts=$(date +%s%N)
...
# Run program
#
output=$(g++ -o /user-solve/main /user-solve/solution.cpp /app/main.cpp -pthread -lm -std=c++11 2>&1)
if [[ $? != 0 ]]; then
