checkstatus() {
    (python3 user-solve/main.py >$1 2>&1)
    if [ ! $? -eq 0 ]; then
        echo "@error@" >>$1
    fi
}
export -f checkstatus
...
dowork() {
...

    {
        ...
        [[ $output =~ ([a-z0-9[:space:]@\"\]*)[[:space:]]?@result@[[:space:]]([a-z0-9@\"\'[:space:]]*) ]]
        ...
        # (/usr/bin/time -o ${result_path} -f ""'{"time" : %e, "memory" : %M}'"" bash -c "$pythoncmd") 2> ${error_path}

        # (/usr/bin/time -o ${result_path} -f ""'{"time" : %e, "memory" : %M}'"" bash -c "checkstatus $output_path $2") 2> ${error_path}
    } || {
